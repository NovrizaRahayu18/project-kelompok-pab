...
// Route penjualan
router.get('/api/penjualan', verifyToken, checkRole(['owner','admin', 'kasir', 'supervisor']), getPenjualan);
router.get('/api/penjualan/:id', verifyToken, 
    checkRole(['owner','admin'['owner','admin', 'kasir', 'supervisor']]), getPenjualanById);

// Ini untuk detail penjualan
router.get("/api/detail-penjualan/:id", verifyToken,
  checkRole(["owner", "admin"[("owner", "admin", "kasir", "supervisor")]]),getDetailPenjualanById
);

// Route untuk membuat penjualan baru
router.post("/api/penjualan", verifyToken,
  checkRole(["owner", "admin"[("owner", "admin", "kasir", "supervisor")]]), createPenjualan
);

// Cek barang sudah dipakai di penjualan
router.get('/api/barang/check-penjualan/:id', verifyToken, 
    checkRole(['owner','admin'['owner','admin', 'kasir', 'supervisor']]), checkBarangInSales);

router.get('/api/dashboard-stats', verifyToken, 
    checkRole(['owner','admin'['owner','admin', 'kasir', 'supervisor']]), getDashboardStats);
